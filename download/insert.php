...
<?
	...
	// 다중 파일 업로드
	$files = $_FILES["upfile"];
	$count = count($files["name"]);	 //count, file몇 개 첨부되어있는지		
	$upload_dir = './data/';
	...
	for ($i=0; $i<$count; $i++)
	{
		...
		$file = explode(".", $upfile_name[$i]);   //a1.txt 점을기준으로 앞에 파일명 빼는 explode
		$file_name = $file[0];   //a1
		$file_ext  = $file[1];   //.txt

		if (!$upfile_error[$i])   //위에 파일명을 빼고 연월일시분초 단위로 이름을 넣어준다
		{
			$new_file_name = date("Y_m_d_H_i_s");
			$new_file_name = $new_file_name."_".$i;
			$copied_file_name[$i] = $new_file_name.".".$file_ext;      
			$uploaded_file[$i] = $upload_dir.$copied_file_name[$i];
                //$uploaded_file[$i] = './data/2020_10_14_10_12_24_0.txt'
